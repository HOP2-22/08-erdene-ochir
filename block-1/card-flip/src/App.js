...
const App = () => {
  const characters = ["ch", "bit", "ck", "fu"];
  const [flipped, setFlipped] = useState(
    new Array(characters.length * 2).fill(false)
  );
  const [cards, setCards] = useState([]);
  const flip = (index) => {
    let newFlipped = flipped.map((f, i) => (i === index ? !f : f));
    let flippedCount = newFlipped.filter((flip) => flip == true).length;
    if (flippedCount == 3) {
      setFlipped(new Array(characters.length * 2).fill(false));
    } else {
  ...
  const generate = () => {
    ...
    setCards(newCards);
    // ["mitsuri", "rengoku", "sanemi", "shinobu", "tanjiro", "mitsuri", "rengoku", "sanemi", "shinobu", "tanjiro"];
  };
  useEffect(() => {
    generate();
  }, []);
